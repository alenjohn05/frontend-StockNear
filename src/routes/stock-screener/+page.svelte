...
<section class="w-full max-w-4xl overflow-hidden m-auto min-h-screen pt-5 pb-40">
  ...
  <div class="w-full max-w-4xl m-auto sm:bg-[#0d1117] sm:rounded-xl h-auto pl-10 pr-10 pt-5 sm:pb-10 sm:pt-10 mt-3 mb-8">
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-10">
      ...
      <div>
        ...
        <div class="hidden sm:flex flex-col justify-center items-center m-auto pt-10 mb-10 sm:mb-0">
          <label for={data?.user ? "addStrategy": "userLogin"} class="flex flex-row items-center justify-center w-56 sm:w-72 hover:bg-[#161b22] bg-[#161b22] bg-opacity-[0.6] duration-150 cursor-pointer py-2.5 px-4 font-medium text-center text-white rounded-full">
    ...
    {#if isLoaded}
      ...
      <div class=" sm:rounded-xl shadow-lg bg-[#0d1117] pt-5 pb-5 sm:pb-0 mt-5">
        ...
        <div class="flex flex-row {$screenWidth >= 640 ? '-mb-4' : ''} ">
          ...
          <div class="w-3/4 flex ml-auto flex-row justify-end items-center -mt-2">
            <div class="flex flex-col justify-center items-center mr-2">
              <label on:click|stopPropagation={() => handleView(st?.rules,st?.title)} for="viewStrategy" class="cursor-pointer w-8 h-8 bg-[#0A59F5] hover:bg-[#0844BC] hover:ring-[1px] hover:bg-[#161b22] bg-[#161b22] bg-opacity-[0.6] duration-150 font-medium text-center text-white rounded-full">
            ...
            <div class=" flex flex-col justify-center items-center ml-2 mr-2">
              <label on:click|stopPropagation={() => selectStrategy(st?.id)} class="cursor-pointer w-8 h-8 bg-[#0A59F5] hover:bg-[#0844BC] hover:ring-[1px] hover:bg-[#161b22] bg-[#161b22] bg-opacity-[0.6] duration-150 font-medium text-center text-white rounded-full">
            ...
            <div class=" flex flex-col justify-center items-center ml-2 mr-8">
              <label on:click|stopPropagation={() => deleteStrategyId= st?.id} for="deleteStrategy" class="cursor-pointer w-8 h-8 bg-[#0A59F5] hover:bg-[#0844BC] hover:ring-[1px] hover:bg-[#161b22] bg-[#161b22] bg-opacity-[0.6] duration-150 font-medium text-center text-white rounded-full">
      ...
      <div class="relative p-2 sm:hidden -mt-2">
        {#each strategyList as item, index}
          <div class="bg-[#0d1117] shadow-lg rounded-lg border border-slate-800 h-fit pb-2 pl-4 pr-4 pt-4 mb-7">
              ...
              <div class="sm:hidden w-1/2 mt-5 m-auto flex flex-row justify-center items-center">
                <div class="flex flex-col justify-center items-center mr-5">
                  <label on:click|stopPropagation={() => handleView(item?.rules,item?.title)} for="viewStrategy" class="cursor-pointer w-12 h-12 bg-[#0A59F5] hover:bg-[#0844BC] hover:ring-[1px] hover:bg-[#161b22] bg-[#161b22] bg-opacity-[0.6] duration-150 font-medium text-center text-white rounded-full">
                ...
                <div class=" flex flex-col justify-center items-center ml-5 mr-5">
                  <label on:click|stopPropagation={() => goto("/stock-screener/"+item?.id)} class="cursor-pointer w-12 h-12 bg-[#0A59F5] hover:bg-[#0844BC] hover:ring-[1px] hover:bg-[#161b22] bg-[#161b22] bg-opacity-[0.6] duration-150 font-medium text-center text-white rounded-full">
                ...
                <div class=" flex flex-col justify-center items-center ml-5">
                  <label on:click|stopPropagation={() => deleteStrategyId= item?.id} for="deleteStrategy" class="cursor-pointer w-12 h-12 bg-[#0A59F5] hover:bg-[#0844BC] hover:ring-[1px] hover:bg-[#161b22] bg-[#161b22] bg-opacity-[0.6] duration-150 font-medium text-center text-white rounded-full">
    ...
    <div class="{!data?.user ? 'hidden' : ''} sm:hidden fixed z-50 w-full h-16 max-w-3xl -right-5 bottom-5">
      <div class="h-full max-w-3xl mx-auto">        
        <div class="flex items-center justify-end mr-10">
          <label for="addStrategy" class="inline-flex items-center justify-center w-14 h-14 font-medium bg-[#161b22] bg-opacity-[0.6] rounded-full cursor-pointer">
  ...
  <dialog id="addStrategy" class="modal modal-bottom sm:modal-middle">
    ...
    <label for="addStrategy"  class="cursor-pointer modal-backdrop bg-[#0d1117] bg-opacity-[0.5]"></label>
    ...
    <div class="modal-box w-full bg-[#0d1117] border border-[#2a2e39]" >
      ...
      <form
        ...
        <button type="submit" class="mt-10 btn bg-[#161b22] hover:bg-purple-500 btn-md w-full rounded-lg m-auto text-white font-bold text-md">
          Create Strategy
        </button>
  ...
  <dialog id="editStrategy" class="modal modal-bottom sm:modal-middle">
    ...
    <div class="modal-box w-full" >
      ...
      <ul class="mt-3 menu menu-compact text-[#A4AAB8] bg-[#0d1117] rounded-md pt-3">
        <!--<hr>-->
        <li class="mb-3">
  ...
  <dialog id="deleteStrategy" class="modal modal-bottom sm:modal-middle">
    ...
    <label for="deleteStrategy"  class="cursor-pointer modal-backdrop bg-[#0d1117] bg-opacity-[0.5]"></label>
    
    
    <div class="modal-box w-full bg-[#0d1117] border border-[#2a2e39] overflow-hidden ">
      ...
      <label for="deleteStrategy" on:click={handleDeleteStrategy} class="mt-5 btn bg-[#161b22] hover:bg-purple-500 btn-md w-full rounded-lg m-auto text-white font-bold text-md">
          Proceed
      </label>
  ...
  <dialog id="viewStrategy" class="modal modal-bottom sm:modal-middle">
    ...
    <label for="viewStrategy"  class="cursor-pointer modal-backdrop bg-[#0d1117] bg-opacity-[0.5]"></label>
    
    
    <div class="modal-box w-full bg-[#0d1117] border border-[#2a2e39] border border-slate-600 overflow-y-scroll overflow-hidden {$screenWidth < 640 ? 'min-h-screen' : 'h-[800px]'}">
...
<div class="drawer drawer-end overflow-hidden" style="z-index: 9999">
  ...
  <div class="w-screen min-h-full bg-[#0d1117] text-base-content overflow-y-scroll overflow-hidden p-2 pb-10">
    <!-- Search layout -->
    ...
    {#each ruleOfList as rule,index}
          <div class="pl-3 pr-3 pb-2 text-white">
            <div class="flex flex-row items-center w-full bg-[#0d1117] p-3 rounded-lg">
                <div class="text-sm w-full mt-2">
                  <span class="mr-1">{index+1}.</span>
                  <!--<svg class="flex-shrink-0 w-5 h-5 sm:w-6 sm:h-6 text-green-400 indine-block" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path></svg>-->
                  {ruleMappings[rule.name] || rule.name} · {formatRuleValue(rule)}
