  ...
  if($page.url.pathname)
  {
      ...
      <body class="bg-[#131722] pb-40">
          ...
          <div class="flex flex-col min-h-screen overflow-hidden m-auto w-full max-w-8xl mt-5 supports-[overflow:clip]:overflow-clip pb-40">
             ...
             <!--Start Mobile Navbar-->
             <div class="navbar w-screen sticky top-0 z-30 bg-[#131722] sm:hidden">
                  ...
                  <div class="pb-12 md:pb-20 w-full max-w-5xl lg:max-w-3xl">
                      <div class="md:pr-6 lg:pr-10">
                                      ...
                                      <div class="m-auto pl-0 sm:pl-4 max-w-3xl overflow-hidden mb-5 md:mt-10">
                                        ...
                                        <div class="hidden sm:flex flex-row w-full justify-between items-center pb-10">
                                            ...
                                            <div class="flex flex-col ml-auto mr-2">
                                              {#if userWatchList?.length !== 0}
                                              <div class="flex-shrink-0 rounded-full hover:bg-white hover:bg-opacity-[0.02] transition ease-out w-12 h-12 relative bg-[#131722] flex items-center justify-center">
                                                <label for="addWatchListModal" class="cursor-pointer flex-shrink-0">
                                                  {#if isTickerIncluded}
                                            ...
                                            {:else}
                                            <div class="flex-shrink-0 ml-auto mr-2 rounded-full hover:bg-white hover:bg-opacity-[0.02] transition ease-out w-12 h-12 relative bg-[#131722] flex items-center justify-center">
                                      ...
                                      <div class="ml-auto sm:hidden">
                                        <Markethour />
                                            ...
                                            <div class="hidden sm:flex ml-auto">
                                              ...
                                              <label for="{!data?.user  ? 'userLogin' : userPortfolio?.length !== 0 ? 'typeOfTrade' : ''}" class="py-2 px-3 text-sm sm:text-[1rem] cursor-pointer mr-1 flex flex-row ease-in-out duration-100 rounded-full shadow-lg bg-[#131722] hover:bg-[#2A303C] border border-slate-800 normal-case cursor-pointer items-center">
                                              ...
                                              {:else}
                                              <label for="{!data?.user  ? 'userLogin' : userPortfolio?.length === 0 ? 'addPortfolio' : 'buyTradeModal'}" class="py-2 px-3 text-sm sm:text-[1rem] cursor-pointer mr-1 flex flex-row ease-in-out duration-100 rounded-full shadow-lg bg-[#131722] hover:bg-[#2A303C] border border-slate-800 normal-case  cursor-pointer items-center">
                                                <svg class="w-5 h-5 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path fill="white" d="M8 2.5a.5.5 0 0 0-1 0V7H2.5a.5.5 0 0 0 0 1H7v4.5a.5.5 0 0 0 1 0V8h4.5a.5.5 0 0 0 0-1H8z"/></svg>
                                                <span class="text-white font-medium">
                                                  Portfolio
                                                </span>
                                      ...
                                      <!-----End-Header-CandleChart-Indicators------>
                                        ...
                                        <!--<div class="w-full max-w-3xl sm:max-w-2xl m-auto pt-2 pb-5 sm:pl-3 sticky z-20 bg-[#131722]"  style="top: {$screenWidth < 520 && $isScrollingUp ? '4rem' : '0rem'};">-->
                                          <div class="sm:ml-4 w-screen sm:w-full {$screenWidth < 640 ? 'overflow-auto scrollbar no-scrollbar' : ''} mb-2" >
                                            <ul class="pr-4 sm:pr-0 w-screen font-medium flex flex-row items-center bg-[#131722]  space-x-3 rtl:space-x-reverse py-2">
                                              <li class="cursor-pointer flex flex-col items-center">
                                                <a href={`/etf/${$etfTicker}`} id="item1" on:click={() => (changeSection('overview','item1'))} class="px-2 text-sm sm:text-[0.9rem] font-medium text-gray-400 sm:hover:text-white {displaySection === 'overview' ? 'text-white ' : 'bg-[#131722]'}" >
                                                  Overview
                                                </a>
                                                <div class="{displaySection === 'overview' ? 'bg-[#75D377]' : 'bg-[#131722]'} mt-1 h-[3px] rounded-full w-[3.5rem]" />
                                              </li>
                                              <li class="cursor-pointer flex flex-col items-center">
                                                <a href={`/etf/${$etfTicker}/stats`} id="item2" on:click={() => (changeSection('stats','item2'))} class="px-2 text-sm sm:text-[0.9rem] font-medium text-gray-400 sm:hover:text-white {displaySection === 'stats' ? 'text-white ' : 'bg-[#131722]'}" >
                                                  Stats
                                                </a>
                                                <div class="{displaySection === 'stats' ? 'bg-[#75D377]' : 'bg-[#131722]'} mt-1 h-[3px] rounded-full w-[2rem]" />
                                              </li>
                                              <li class="cursor-pointer flex flex-col items-center">
                                                <a href={`/etf/${$etfTicker}/options`} id="item3" on:click={() => (changeSection('options','item3'))} class="px-2 text-sm sm:text-[0.9rem] font-medium text-gray-400 sm:hover:text-white {displaySection === 'options' ? 'text-white ' : 'bg-[#131722]'}" >
                                                  Options
                                                </a>
                                                <div class="{displaySection === 'options' ? 'bg-[#75D377]' : 'bg-[#131722]'} mt-1 h-[3px] rounded-full w-[3rem]" />
                                              </li>
                                              <li class="cursor-pointer flex flex-col items-center">
                                                <a href={`/etf/${$etfTicker}/holdings`} id="item4" on:click={() => (changeSection('holdings','item4'))} class="px-2 text-sm sm:text-[0.9rem] font-medium text-gray-400 sm:hover:text-white {displaySection === 'holdings' ? 'text-white ' : 'bg-[#131722]'}" >
                                                  Holdings
                                                </a>
                                                <div class="{displaySection === 'holdings' ? 'bg-[#75D377]' : 'bg-[#131722]'} mt-1 h-[3px] rounded-full w-[3rem]" />
                                              </li>
                                              <li class="cursor-pointer flex flex-col items-center">
                                                <a href={`/etf/${$etfTicker}/dividends`} id="item5" on:click={() => (changeSection('dividends','item5'))} class="px-2 text-sm sm:text-[0.9rem] font-medium text-gray-400 sm:hover:text-white {displaySection === 'dividends' ? 'text-white ' : 'bg-[#131722]'}" >
                                                  Dividends
                                                </a>
                                                <div class="{displaySection === 'dividends' ? 'bg-[#75D377]' : 'bg-[#131722]'} mt-1 h-[3px] rounded-full w-[3rem]" />
                                              </li>
                                              <li class="cursor-pointer flex flex-col items-center">
                                                <a href={`/etf/${$etfTicker}/congress-trading`} id="item5" on:click={() => (changeSection('congress-trading','item5'))} class="px-2 text-sm sm:text-[0.9rem] font-medium text-gray-400 sm:hover:text-white {displaySection === 'congress-trading' ? 'text-white ' : 'bg-[#131722]'}" >
                                                  Insider
                                                </a>
                                                <div class="{displaySection === 'congress-trading' ? 'bg-[#75D377]' : 'bg-[#131722]'} mt-1 h-[3px] rounded-full w-[3rem]" />
                                              </li>
                                              <li class="cursor-pointer flex flex-col items-center pr-6">
                                                <a href={`/etf/${$etfTicker}/news`} id="item7" on:click={() => (changeSection('news','item7'))} class="px-2 text-sm sm:text-[0.9rem] font-medium text-gray-400 sm:hover:text-white {displaySection === 'news' ? 'text-white ' : 'bg-[#131722]'}" >
                                                  News
                                                </a>
                                                <div class="{displaySection === 'news' ? 'bg-[#75D377]' : 'bg-[#131722]'} mt-1 h-[3px] rounded-full w-[2rem]" />
                                              </li>
                                            </ul>
                                          </div>
                        ...
                        <div class="sm:hidden fixed z-20 bottom-8 sm:bottom-10 right-5">
                          <div class="h-full mx-auto">        
                            <div class="flex items-center justify-end">
                              {#if holdingShares !== 0 && data?.user}
                              <label for="{!data?.user  ? 'userLogin' : userPortfolio?.length !== 0 ? 'typeOfTrade' : ''}" class="inline-flex items-center justify-center w-32 h-11 border border-[#000] ring-[#000] bg-[#0DDE00] text-[0.95rem] font-medium rounded-full text-[#131722]">
                                Invest
                              </label>
                              {:else}
                              <label for="{!data?.user  ? 'userLogin' : userPortfolio?.length === 0 ? 'addPortfolio' : 'buyTradeModal'}"
                              class="inline-flex items-center justify-center w-32 h-11 border border-[#000] ring-[#000] bg-[#0DDE00] text-[0.95rem] font-medium rounded-full text-[#131722]">
                                Invest
                              </label>
                              {/if}
                            </div>
...
<!--End Add Portfolio Modal-->
  ...
  <dialog id="addWatchListModal" class="modal modal-bottom sm:modal-middle ">
    ...
    <label for="addWatchListModal" class="cursor-pointer absolute right-5 top-2 bg-[#131722] text-[1.8rem] text-white">
      ✕
    </label>
