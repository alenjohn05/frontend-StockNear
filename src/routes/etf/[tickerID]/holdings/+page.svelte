                  ...
                  <div class="mb-6">
                    ...
                    <div class="flex justify-start items-center m-auto overflow-x-auto">
                      ...
                      <table class="table table-pin-cols table-sm table-compact rounded-none sm:rounded-md w-full border-bg-[#0d1117] m-auto mt-4 overflow-x-auto">
                          <thead>
                            <tr class="">
                              <td class="text-white border-b border-[#141720] bg-[#0d1117] font-semibold text-sm">No.</td>
                              <td class="text-white border-b border-[#141720] bg-[#0d1117] font-semibold text-sm">Symbol</td>
                              <td class="text-white border-b border-[#141720] bg-[#0d1117] font-semibold text-sm hidden sm:table-cell">Name</td>
                              <td class="text-white border-b border-[#141720] bg-[#0d1117] font-semibold text-end text-sm ">Shares</td>
                              <td class="text-white border-b border-[#141720] bg-[#0d1117] font-semibold text-end text-sm">% Weight</td>
                            </tr>
                          </thead>
                          <tbody>
                        ...
                        <!-- row -->
                        {#if item?.asset !== null}
                        <tr on:click={() => stockSelector(item?.asset)} class="w-full sm:hover:bg-[#245073] sm:hover:bg-opacity-[0.2] odd:bg-[#0d1117] {index+1 === holdings?.slice(0,3)?.length && data?.user?.tier !== 'Pro' ? 'opacity-[0.1]' : ''} {item?.asset?.length !== 0 ? 'cursor-pointer' : ''}">
                          
                          <td class="text-gray-200 border-b border-[#141720]">
                            {index+1}
                          </td>

                          <td class="font-normal  border-b border-[#141720]">
                            <div class="flex flex-col items-start">
                              <span class="text-[#FFBE00]">{item?.asset}</span>
                          ...
                          <td class="text-gray-200 border-b border-[#141720] w-fit hidden sm:table-cell">
                            {item?.name?.length > charNumber ? formatString(item?.name?.slice(0,charNumber)) + "..." : formatString(item?.name)}
                          </td>
            
          
              
                        <td class="text-gray-200 border-b border-[#141720] text-end ">
