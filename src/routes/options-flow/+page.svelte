...
$: {
  ...
  <title>
    {$numberOfUnreadNotification > 0 ? `(${$numberOfUnreadNotification})` : ''} Options Flow Feed · octopi
  </title>
  <meta name="description" content={`Explore unusual options from big institutional traders and hedge funds.`} />
  
  <!-- Other meta tags -->
  <meta property="og:title" content={`Options Flow Feed · octopi`}/>
  ...
  <meta name="twitter:title" content={`Options Flow Feed · octopi`}/>
...
<body class="sm:fixed h-screen m-auto w-full max-w-screen">
  <section class="w-full max-w-screen sm:max-w-screen-2xl flex justify-center items-center m-auto pt-5 bg-[#0d1117] ">
      ...
      <div class="w-full m-auto mb-10 pl-3 pr-3">
        ...
        {/if}
          ...
          <div class="flex flex-col sm:flex-row items-center w-full bg-[#0d1117] border border-[#2a2e39] rounded-lg px-3">
              ...
              <div class="flex flex-row items-center justify-center sm:justify-start mt-6 pb-5">
                <label data-tip="Audio Preference" on:click={() => muted = !muted} class="xl:tooltip xl:tooltip-bottom flex flex-col items-center mr-6 cursor-pointer">
                  <div class="rounded-full w-10 h-10 relative bg-[#0d1117] flex items-center justify-center">
                    {#if !muted}
          ...
          <div class="sm:ml-auto w-full sm:w-fit">
            <div class="relative flex flex-col sm:flex-row items-center">
              ...
              <div class="py-2 sm:py-1.5 mb-5 sm:mb-0 flex-auto text-center bg-[#0d1117] rounded-lg w-full sm:w-fit">
          ...
          <div class="w-full mt-5 mb-10 m-auto flex justify-center items-center">
            <div class="w-full grid grid-cols-1 lg:grid-cols-4 gap-y-3 gap-x-3 ">
              ...
              <div class="flex flex-row items-center flex-wrap w-full px-5 bg-[#0d1117] border border-[#2a2e39] shadow-lg rounded-lg h-20">
            ...
            {#if showMore}
             <!--Start Most Traded-->  
             <div class="flex flex-row items-center flex-wrap w-full px-5 bg-[#0d1117] shadow-lg rounded-lg h-20">
              <div class="flex flex-col items-start">
              ...
              <div bind:this={scrollContainer} class="mt-4 w-full overflow-x-auto overflow-y-auto h-[900px] rounded-lg">
                <table class="table table-pin-cols table-pin-rows table-sm table-compact">
                    ...
                    <tbody>
                      ...
                      <!-- row -->
                      <tr on:click={() => handleViewData(item)} class="w-full odd:bg-[#0d1117] cursor-pointer {index+1 === optionList?.length && data?.user?.tier !== 'Pro' ? 'opacity-[0.1]' : ''}">
                        
                        <td class="text-white pb-3 text-xs sm:text-sm text-start">
                          {formatTime(item?.time)}
                        </td>
    
                        <th on:click|stopPropagation={() => assetSelector(item?.ticker, item?.assetType)} class="{index % 2 ? 'bg-[#0d1117]' : 'bg-[#0d1117]'} text-[#FFBE00] text-start font-normal">
                          {item?.ticker}
                        </th>
...
<dialog id="filterList" class="modal modal-bottom sm:modal-middle ">
  ...
  <div class="modal-box w-full bg-[#0d1117] border border-[#2a2e39] sm:border sm:border-slate-800 overflow-hidden rounded-md">
...
<label for="optionDetailsDesktopModal" class="hidden sm:modal modal-bottom sm:modal-middle cursor-pointer">
  ...
  <label class="modal-box w-full relative bg-[#0d1117] h-auto max-h-[900px] overflow-y-scroll">
    <label for="optionDetailsDesktopModal" class="cursor-pointer absolute right-5 top-2 bg-[#0d1117] text-2xl text-white">
      ✕
    </label>
