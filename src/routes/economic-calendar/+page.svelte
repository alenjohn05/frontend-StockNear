...
<section class="w-full max-w-4xl overflow-hidden m-auto min-h-screen pt-5 pb-40">
          ...
          <div class="w-full max-w-4xl m-auto sm:bg-[#0d1117] sm:rounded-xl h-auto sm:p-10 mt-10 sm:mt-3 mb-8">
            ...
            <div class="flex justify-center w-full max-w-5xl m-auto h-full overflow-hidden">
                ...
                <div class=" w-full flex flex-row justify-center m-auto items-center pl-2 pr-2 sm:pl-0 sm:pr-0">
                    <!-- Start Columns -->
                    <label on:click={() => changeWeek('previous')} class="{previousMax ? 'opacity-80' : ''} hidden sm:flex h-16 w-48 cursor-pointer border m-auto flex bg-[#3C40F0] hover:bg-[#161b22] border border-blue-600 mb-3">
                    ...
                    <div class="w-full {index === selectedWeekday ? '' : 'hidden sm:block'}">
                            <label on:click={() => toggleDate(index)} class="w-11/12 m-auto sm:w-full cursor-pointer h-16 {index === selectedWeekday ? 'bg-[#161b22] bg-opacity-[0.6] sm:bg-[#A24D51] sm:gradient-effect' : ''} rounded-lg sm:rounded-none flex bg-[#3C40F0] border border-blue-600 mb-3">
                    ...
                    {/each}
                    <label on:click={() => changeWeek('next')} class="{nextMax ? 'opacity-80' : ''} hidden sm:flex h-16 w-48 cursor-pointer border m-auto flex bg-[#3C40F0] hover:bg-[#161b22] border border-blue-600 mb-3">
...
{#if filterList?.length !== 0}
<label on:click={() => filterList = [] } class="mr-3 text-sm cursor-pointer bg-[#0d1117] sm:hover:bg-[#0d1117] duratiion-100 transition ease-in-out px-4 py-2 rounded-lg shadow-lg ml-auto">
...
<div class="dropdown dropdown-end z-30">
  <button tabindex="0" role="button" class="text-sm cursor-pointer bg-[#0d1117] sm:hover:bg-[#0d1117] duratiion-100 transition ease-in-out px-4 py-2 rounded-lg shadow-lg ml-auto">
    Filter
  ...
  <ul tabindex="0" class="z-30 dropdown-content p-2 shadow bg-base-100 rounded w-60 h-72 oveflow-hidden overflow-y-scroll">
    ...
    <div class="p-3">
      ...
      <div class="relative">
        ...
        <input type="text" id="input-group-search" class="block w-full p-2 ps-10 text-sm placeholder-white text-white border border-gray-300 rounded-lg bg-[#0d1117] focus:ring-blue-500 focus:border-blue-500 " placeholder="Search Country">
      </div>
    </div>
    -->
    ...
    <li>
      <label on:click|stopPropagation={(event) => handleFilter(event, item)} class="flex items-center ps-2 rounded">
        <input checked={filterList?.includes(item)} type="checkbox" class="w-4 h-4 text-[#FFBE00] bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2">
    ...
    <li>
      <div on:click|stopPropagation={(event) => handleFilter(event, item)} class="flex items-center ps-2 rounded">
        <input type="checkbox" checked={filterList?.includes(item)} class="w-4 h-4 text-[#FFBE00] bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2">
        <label class="w-full py-2 ms-2 text-sm font-medium text-white rounded cursor-pointer">
          {item}
        </label>
      </div>
        ...
        {#each (filterList?.length === 0 ? weekday : weekdayFiltered) as day,index}
                  {#if index === selectedWeekday}
                    ...
                    <div class="w-full overflow-x-scroll no-scrollbar">  
                      <table class="hidden sm:inline-table table-sm table-compact rounded-none sm:rounded-md w-full border-bg-[#0d1117] m-auto mt-4 ">
                          <thead>
                            <tr>
                          ...
                          <tbody>
                            {#each day as item}
                            <!-- row -->
                            <tr class="sm:hover:bg-[#245073] sm:hover:bg-opacity-[0.2] odd:bg-[#0d1117]">
                              
                            <td class="text-white text-sm border-b-[#0d1117]">
                            ...
                            <td class="text-start text-white border-b-[#0d1117]">
                          ...
                          <td class="text-end text-white font-medium text-center border-b-[#0d1117] text-end">
                            {item?.actual !== null ? item?.actual : '-'}
                          </td>
          
                          <td class="{item?.impact === 'Low' ? 'text-[#00FC50]' : item?.impact === 'Medium' ? 'text-[#3DDBFE]' : item?.impact === 'High' ? 'text-[#FC2120]' : 'text-white'} text-sm text-end mr-1 border-b-[#0d1117]">
                            {item?.impact}
                          </td>
                        ...
                        <div class="relative p-2 sm:hidden pt-5 ">
                          {#each day as item}
                            <div class="bg-[#0d1117] rounded-lg border border-slate-800 h-auto pl-2 pr-2 pt-4 pb-3 mb-7">
