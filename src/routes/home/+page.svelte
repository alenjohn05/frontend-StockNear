<script lang="ts">
  ...
  const marketMoverData = data?.getDailyGainerLoserActiveOcto;
  const getlatestRssNews = data?.getlatestRssNews?.GetLiveNews?.data || [];
  const getliveMarketCommentary =
    data?.getlatestRssNews?.liveMarketCommentary?.data || [];
  let active = rawData?.active["1D"]?.slice(0, 5);
  let oct_gainer = marketMoverData?.GetPriceMoversGainers.slice(0, 5);
  let oct_loser = marketMoverData?.GetPriceMoversLosers.slice(0, 5);
  let oct_active = marketMoverData?.GetVolumeShockers.slice(0, 5);
  //let sliderList = rawData?.active['1D'];

  let gainerLoserTickers: any[] = [];
  let showLoser = false;
  let buttonText = "Top Winners";
  let trendingText = "gainer";
  gainerLoserTickers = oct_gainer;
...
<svelte:head>
  ...
  <title>
    ...
    Stock Analysis Information for Small Investors · octopi
  </title>

  <meta
    name="description"
    content="Octopi has everything you need to analyze stocks with help of AI, including detailed financial data, statistics, news and charts."
  />
  <!-- Other meta tags -->
  <meta
    property="og:title"
    content="Free Stock Analysis Information for Small Investors · octopi"
  />
  <meta
...
<div class="w-full max-w-6xl overflow-hidden m-auto min-h-screen bg-[#0d1117]">
  <div
      ...
      <div class="text-center mb-10 w-full">
        ...
        <section>
          <div class="relative w-full max-w-6xl mx-auto overflow-hidden">
            ...
            <div
              class="w-full max-w-5xl m-auto flex justify-center overflow-hidden"
            >
              <div class="py-7">
                <!-- Section content -->
                      ...
                      <div class="grid grid-cols-1 gap-10">
                        ...
                        <div>
                          {#if (data?.user?.tier === "Pro" && data?.user?.freeTrial === true) || (data?.user?.tier !== "Pro" && data?.user?.freeTrial === false)}
                            <div
                              class="text-center mb-5 relative w-fit flex justify-center m-auto"
                            >
                              <a
                                href="/pricing"
                                class="text-white antialiased bg-[#0d1117] w-full px-4 py-2 rounded-xl m-auto font-medium text-sm flex items-center"
                              >
                              ...
                              <div
                                class="absolute top-[-1.2rem] -right-5 sm:-right-8 rotate-[7deg]"
                              >
                                <span
                          ...
                          >
                            <h1
                              class="text-center text-4xl sm:text-5xl text-white font-bold mb-6 w-full"
                            >
                          ...
                          <span
                            class="text-center flex-1 justify-center items-center text-white text-[1rem] sm:text-lg"
                          >
                            Track the forces driving the stock market with our
                            actionable and easy-to-understand data.
                          </span>

                          <div
                            class="flex justify-center items-center mt-8 mb-6 m-auto w-full max-w-64 sm:max-w-3xl"
                          >
                            <!-- svelte-ignore a11y-click-events-have-key-events -->
                            <label
                              on:click={loadSearchData}
                              for="searchBarModal"
                              class="shadow-lg w-96 h-10 sm:h-12 flex flex-row items-center justify-start bg-[#0d1117] appearance-none py-3 cursor-pointer border border-slate-600 rounded-lg"
                            >
                              <svg
                              ...
                              <span
                                class="text-sm sm:text-lg text-slate-200 ml-3"
                              >
                                Explore Stocks and MFs...
                              </span>
                            </label>
                          </div>
                            ...
                            >
                              ...
                              {#each active?.slice(0, 4) as item, index}
                                <a
                                  ...
                                  class="ml-1 text-[#FFBE00] sm:hover:text-slate-100"
                                >
                                  {item?.symbol}
                                </a>
                                {#if index !== 3}
                                  <span class="text-[#FFBE00]">,</span>
                                {/if}
                              {/each}
                            </div>
                          {/if}
                          ...
                          {#if data?.user}
                            <div
                              class="text-white text-center text-[1rem] sm:text-lg mt-8 mb-4 font-medium"
                            >
                              Track the investment portfolios of prominent
                              Figures.
                            </div>

                            <div
                              class="w-full max-w-64 sm:max-w-3xl m-auto grid grid-cols-1 md:grid-cols-3 gap-y-3 sm:gap-x-5 flex justify-center items-center"
                            >
                              <a
                                href="/politicians/61b59ab669"
                                class="px-4 py-3 border border-[#2a2e39] text-white bg-[#0d1117] rounded-lg flex flex-row items-center font-medium transition duration-150 ease-in-out group"
                              >
                                <div class="flex flex-row items-center">
                                  <div
                                    class="shadow-lg rounded-full border border-slate-600 w-10 h-10 relative democratic-striped bg-[#295AC7] flex items-center justify-center"
                                  >
                                    <img
                                      style="clip-path: circle(50%);"
                                      class="rounded-full w-8"
                                      src={"https://upload.wikimedia.org/wikipedia/commons/thumb/e/ea/Shri_Rakesh_Radheyshyam_Jhunjhunwala.jpg/640px-Shri_Rakesh_Radheyshyam_Jhunjhunwala.jpg"}
                                      loading="lazy"
                                      alt=""
                                    />
                                  </div>
                              ...
                              >
                                <div class="flex flex-row items-center">
                                  <div
                                    class="shadow-lg rounded-full border border-slate-600 w-10 h-10 relative hedge-fund-striped bg-[#20202E] flex items-center justify-center"
                                  >
                                    <img
                                      style="clip-path: circle(50%);"
                                      class="rounded-full w-8"
                                      src={warrenBuffetProfile}
                                      loading="lazy"
                                    />
                                  </div>
                                  <div class="flex flex-col items-start ml-2">
                              ...
                              <a
                                ...
                                class="px-4 py-3 border border-[#2a2e39]
                                      text-white bg-[#0d1117] rounded-lg flex flex-row items-center font-medium transition duration-150 ease-in-out group"
                              >
                                <div class="flex flex-row items-center">
                                  <div
                                    class="shadow-lg rounded-full border border-slate-600 w-10 h-10 relative analyst-striped bg-[#737D62] flex items-center justify-center"
                                  >
                                    <img
                                      style="clip-path: circle(50%);"
                                      class="rounded-full w-8"
                                      src={"https://www.enam.com/img/enam-logo.jpg"}
                                      loading="lazy"
                                    />
                                  </div>
                                  <div class="flex flex-col items-start ml-2">
            ...
            <div
              class="w-full max-w-3xl m-auto mb-20 -mt-4 flex justify-center items-center"
            >
              <a
                href="/community"
                class="px-4 py-3 text-white bg-[#0d1117] border border-slate-800 border border-[#2a2e39] rounded-lg flex flex-row items-center font-medium transition duration-150 ease-in-out group"
              >
                <svg
                  class="w-6 h-6 mr-2"
                  viewBox="0 0 24 24"
                ...
                <span
                  class="ml-1 mt-0.5 tracking-normal group-hover:translate-x-0.5 transition-transform duration-150 ease-in-out"
                >
                  <svg
                    class="w-4 h-4"
                    xmlns="http://www.w3.org/2000/svg"
            ...
            <div
              class="grid grid-cols-1 lg:grid-cols-2 lg:gap-x-10 w-full m-auto pb-14 sm:pb-20 sm:mt-10"
            >
              <div class="hidden sm:block w-full">
                ...
                >
                  <button on:click={handleClick}>
                    <h2
                      class="text-white text-xl sm:text-2xl font-bold text-start mb-2 ml-2 sm:ml-0"
                    >
                      <svg
                        class="h-6 w-6 inline-block transform transition-transform {showLoser
                          ? 'mb-2'
                          : 'rotate-180'} mr-1"
                        xmlns="http://www.w3.org/2000/svg"
                ...
                >
                  <thead class="rounded-lg">
                    <tr class="bg-[#0d1117] border-b border-[#2a2e39]">
                  ...
                  <tbody>
                    {#each gainerLoserTickers as item}
                      <tr
                        ...
                        class="sm:hover:bg-[#245073] sm:hover:bg-opacity-[0.2] odd:bg-[#0d1117] border-b border-[#2a2e39] shake-ticker cursor-pointer"
                      >
                        <td class="text-white">
                          {item?.Name?.length > 50
                            ...
                            <div class="flex flex-col items-center">
                              ...
                              <div class="flex flex-row mt-1">
                                {#if item?.CZG >= 0}
                                  ...
                                  >
                                  <span
                                    class="text-[#10DB06] text-xs font-medium"
                                    >+{item?.CZG?.toFixed(2)}%</span
                                  >
                                {:else}
                                  <svg
                                  ...
                                  >
                                  <span
                                    class="text-[#FF2F1F] text-xs font-medium"
                                    >{item?.CZG?.toFixed(2)}%
                                  </span>
                                {/if}
                              </div>
              ...
              <div class="sm:hidden overflow-hidden">
                <div class="flex flex-row items-center w-full">
                  <label
                    class="text-white text-xl p-4 font-medium flex flex-row items-center"
                  >
                    Trending
                    <InfoModal
                      title={"Trending"}
                  ...
                  <a
                    href="/market-mover"
                    class="ml-auto text-slate-300 text-sm font-medium pr-4 cursor-pointer"
                  >
                    See All
                  </a>
                </div>
                ...
                <div
                  class="w-full flex flex-row items-center mb-5 ml-3 overflow-hidden"
                >
                  <label
                    on:click={() => scrollToItem("gainer")}
                    class="flex flex-row items-center {trendingText === 'gainer'
                      ? 'bg-[#333333]'
                      : 'bg-[#0d1117]'} rounded-xl py-1"
                  >
                    <svg
                      class="ml-2 w-5"
                      xmlns="http://www.w3.org/2000/svg"
                  ...
                  <label
                    ...
                    class="flex flex-row items-center {trendingText === 'loser'
                      ? 'bg-[#333333]'
                      : 'bg-[#0d1117]'} rounded-xl py-1 ml-4"
                  >
                    <svg
                      class="ml-2 w-5"
                      xmlns="http://www.w3.org/2000/svg"
                      ...
                      ><path
                        fill="white"
                        d="M240 136v64a8 8 0 0 1-8 8h-64a8 8 0 0 1 0-16h44.69L136 115.31l-34.34 34.35a8 8 0 0 1-11.32 0l-72-72a8 8 0 0 1 11.32-11.32L96 132.69l34.34-34.35a8 8 0 0 1 11.32 0L224 180.69V136a8 8 0 0 1 16 0Z"
                      /></svg
                    >
                    <span class="text-white text-sm font-medium ml-2 mr-4">
                      Losers
                  ...
                  >
                    <svg
                      ...
                      ><path
                        fill="white"
                        d="M253.78 17.188c-130.728 0-236.905 106.177-236.905 236.906C16.875 384.824 123.052 491 253.78 491c130.73 0 236.907-106.18 236.907-236.906c0-130.73-106.177-236.906-236.906-236.906zm0 18.687c120.63 0 218.22 97.59 218.22 218.22c0 120.626-97.59 218.218-218.22 218.218c-120.628 0-218.218-97.59-218.218-218.22s97.59-218.218 218.22-218.218zm101.19 46.313l-76.41 132.875c15.916 9.635 25.177 26.33 26.125 43.78h148.407c1.644-70.01-33.49-138.867-98.125-176.656zm-205.126 2.468c-27.1 16.725-50.68 40.147-67.72 69.656c-19.01 32.928-26.926 69.12-26 104.532H196a54.04 54.04 0 0 1 7.188-24.438c5.21-9.024 12.64-16 21.218-20.625L149.844 84.657zm100.594 141.156a36.723 36.723 0 0 0-2.594.094c-11.446.793-22.288 7.084-28.5 17.844c-9.94 17.216-4.09 38.967 13.125 48.906c17.213 9.94 38.935 4.12 48.874-13.094c9.94-17.215 4.12-38.967-13.094-48.906c-5.648-3.26-11.768-4.824-17.813-4.844zm28.218 82.375c-16.127 9.75-36.864 10.846-54.406 1.25l-68.03 117.22c29.454 16.785 61.044 25.177 92.75 26c34.567.898 68.72-7.786 99.124-24.032l-69.438-120.438z"
                      /></svg
                    >
                    <span class="text-white text-sm font-medium ml-2 mr-4">
                      Most Active
                ...
                <div
                  class="w-screen no-scrollbar grid grid-cols-5 gap-x-44 overflow-hidden overflow-x-scroll pl-5 pr-10"
                >
                  {#each trendingText === "gainer" ? oct_gainer : trendingText === "loser" ? oct_loser : oct_active as item, index}
                    <a
                      href={"/stocks/" + item?.SecurityID}
                      class="bg-[#0d1117] rounded-lg h-[90px] w-36 relative"
                    >
                      ...
                      <div class="flex flex-row items-center justify-end p-2">
                        ...
                        <div
                          class="rounded-full w-7 h-7 relative bg-[#0d1117] flex items-center justify-center"
                        >
                          <img
                            style="clip-path: circle(50%);"
                            class="w-5 h-5"
                      ...
                      <div class="flex flex-col items-center">
                        <span class="text-white font-bold text-xs ml-auto mr-2"
                          >{item.C?.toFixed(2)}</span
                        >
                        <div class="flex flex-row mt-1 ml-auto mr-2">
                          {#if (item?.CZ ?? item?.CZG) >= 0}
                            <svg
                              class="w-5 h-5 -mr-0.5 -mt-0.5"
                              xmlns="http://www.w3.org/2000/svg"
                              viewBox="0 0 24 24"
                          ...
                          {:else}
                            ...
                            <span class="text-[#FF2F1F] text-xs font-medium"
                              >-{item?.CZ?.toFixed(2) ?? item?.CZG?.toFixed(2)}%
                            </span>
                          {/if}
                        </div>
                      </div>
              ...
              <div class="hidden sm:block w-full mt-10 lg:mt-0">
                <div>
                  ...
                  <table
                    class="table table-sm table-compact table-pin-rows shadow-md rounded-none sm:rounded-lg bg-[#0d1117]"
                  >
                    <thead>
                      <tr class="bg-[#0d1117] border-b border-[#2a2e39]">
            ...
            <div class="md:flex">
              <div class="w-full p-2 sm:p-0 mb-20 md:mr-5">
                <h2
                  class="text-white text-xl sm:text-2xl font-bold text-start mt-5 mb-2 ml-2"
                >
                  Latest Market Momentum
                </h2>
                {#each getlatestRssNews?.slice(0, 6) as item}
                  <div
                    class="shadow-md bg-[#0d1117] border border-[#2a2e39] p-6 mb-4 rounded-lg text-start"
                  >
                      ...
                      <div class="flex flex-col items-start">
                        <div class="hidden sm:flex flex-row items-center mb-3">
                          {#if latestInfoDate(item?.Date)}
                            <div
                              class="bg-[#2962ff] text-white font-medium text-xs rounded-lg px-2 py-0.5"
                            >
                              New
                            </div>
                        ...
                        <span class="text-white text-md mt-2"
                          >{item.Caption}</span
                        >
                        <div class="flex flex-col mt-5 items-start w-full">
                          <div
                            class="flex flex-wrap gap-y-3 flex-row items-center"
                          >
                            <div class="mr-2 mb-2 sm:mb-0">
                              <div
                                class="cursor-pointer w-fit bg-[#2a2e39] bg-opacity-[0.5] sm:hover:bg-opacity-[0.6] px-3 sm:px-4 py-1.5 sm:py-2 text-sm rounded-xl hover:text-white text-[#FFBE00]"
                              >
                                {item.sectionname}
                              </div>
                            </div>
                          ...
                          <div
                            class="sm:hidden flex flex-row items-center justify-end mt-3 ml-auto"
                          ></div>
                        </div>
                      </div>
                    </div>
              ...
              <div class="w-full p-2 sm:p-0 mb-20 md:ml-5">
                <h2
                  class="text-white text-xl sm:text-2xl font-bold text-start mt-5 mb-2 ml-2"
                >
                ...
                <div class="shadow-md bg-[#0d1117] border border-[#2a2e39] mb-4 rounded-lg flex flex-col sm:flex-row">
                  <div class="sm:hidden w-full h-48 overflow-hidden rounded-lg">
                    <img
                      class="w-full h-full object-cover rounded-lg"
                      src={item?.IllustrationImage}
                      alt={`Illustration for ${item?.section_name}`}
                      loading="lazy"
                    />
                  </div>
                  <div class="text-sm text-white text-start p-4 sm:p-6 flex-grow">
                    <div class="flex flex-col items-start">
                      ...
                      <span class="text-white text-lg sm:text-xl font-bold leading-tight mb-2">
                        {item?.section_name}
                      </span>
                      <span class="text-white text-md leading-relaxed">
                        {item?.heading}
                      </span>
                    </div>
                  </div>
