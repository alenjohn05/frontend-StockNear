      ...
      <div class="w-screen sm:w-full m-auto">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
          {#if news.length !== 0}
            {#each news as item}
              <article
                class="max-w-xl flex p-3 flex-col w-full mt-5 items-start justify-between bg-[#0d1117] border border-[#2a2e39] shadow-lg h-auto sm:h-[280px] pb-10 sm:pb-5 rounded-none sm:rounded-lg m-auto"
              >
                <div
                ...
                >
                  ...
                  <div class="flex items-center gap-x-4 text-xs">
                    <div
                      class={`relative z-10 code-font rounded-full ${item?.sentiment_category === "negative" ? "bg-red-600" : item?.sentiment_category === "positive" ? "bg-green-600" : "bg-[#161b22]"} px-3 py-1.5 font-medium text-white hover:bg-purple-400`}
                    >
                      {item?.category}
                    </div>
                  </div>
