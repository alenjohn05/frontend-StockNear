<script lang="ts">
  ...
  async function changeWeek(state) {
    ...
    if (getStockSplitsCalendarMain?.length) {
    ...
    <div class="relative flex flex-col flex-1 overflow-hidden">
        ...
        {#each weekday as day, index}
          ...
          >
            <label
              ...
              selectedWeekday
                ? 'bg-purple-600 bg-opacity-[0.6] sm:bg-[#A24D51] sm:gradient-effect'
                : ''} rounded-lg sm:rounded-none flex bg-[#3C40F0] border border-blue-600 mb-3"
            >
      ...
      {#each weekday as day, index}
        {#if index === selectedWeekday}
          ...
          {:else}
            <h2
              class="mt-20 flex justify-center items-center text-xl sm:text-2xl font-bold text-slate-700 mb-5 m-auto"
            >
