<script lang="ts">
  ...
  onMount(() => {
    try {
      sortedData = data?.get_news_items_by_security;
  ...
  <section class="w-full max-w-7xl m-auto sm:bg-[#09090b] sm:rounded-xl h-auto">
    ...
    >
      ...
      >
        ...
        <tbody>
          {#each displayList as item, index}
            <!-- row -->
            <tr
              class="sm:hover:bg-slate-400 sm:hover:bg-opacity-[0.2] bg-[#09090b] border-b border-[#141417] shake-ticker cursor-pointer"
            >
      ...
      {#if dailyEvents.length === 0}
        <div class="text-slate-400 text-xs text-center w-full mt-5">
          <span>No New or Updates Avalible</span>
        </div>
      {/if}
