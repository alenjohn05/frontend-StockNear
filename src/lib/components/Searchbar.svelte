...
class="cursor-pointer w-8 h-8  flex items-center justify-center bg-[#161b22] rounded-full">
      ...
      <div class="mb-3 last:mb-0 mt-3">
        ...
        <ul class="text-sm" >
            {#if !showSuggestions }
              {#each popularList as item}
                <li>
                  <a href={`/${item?.type === 'ETF' ? 'etf' : item?.type === 'Crypto' ? 'crypto' : 'stocks'}/${item?.symbol}`} on:click={() => popularTicker(item?.symbol, item?.assetType) } class="mb-2 {item?.symbol === focusedSuggestion ? 'shake-ticker cursor-pointer flex justify-start items-center p-2 text-white bg-[#2a2e39] bg-opacity-[0.25] rounded group' : 'shake-ticker cursor-pointer bg-[#0d1117] rounded-lg flex justify-start items-center p-2 text-white  group'} w-full">
                    <div class="flex flex-row items-center w-full">
            ...
            <div class="text-start text-sm font-semibold text-slate-400 mb-2">
              ...
              {#each searchResults as item}
                <li>
                  ...
                  <label data-sveltekit-preload-data="false" on:click={() => (searchBarTicker(item?.symbol, item?.type))} class="mb-2 {item?.symbol === focusedSuggestion ? 'shake-ticker cursor-pointer flex justify-start items-center p-2 text-white bg-[#2a2e39] bg-opacity-[0.25] rounded group' : 'cursor-pointer mb-2 bg-[#0d1117] rounded-lg flex justify-start items-center p-2 text-white group'}">
                    <div class="flex flex-row items-center w-full">
    ...
    <div class="mb-3 last:mb-0 mt-3">
      ...
      <ul class="text-sm" >
          {#if !showSuggestions }
            {#each popularList as item}
              <li>
                <a  href={`/${item?.type === 'ETF' ? 'etf' : item?.type === 'Crypto' ? 'crypto' : 'stocks'}/${item?.symbol}`}  on:click={() => popularTicker(item?.symbol, item?.type) } class="mb-2 {item?.symbol === focusedSuggestion ? 'shake-ticker cursor-pointer flex justify-start items-center p-2 text-white bg-[#2a2e39] bg-opacity-[0.25] rounded group' : 'cursor-pointer bg-[#0d1117] bg-opacity-[0.4] rounded-lg flex justify-start items-center p-2 text-white group'} w-full">
                  <div class="flex flex-row items-center w-full">
                    <div class="rounded-full w-10 h-10 relative bg-[#0d1117] flex items-center justify-center">
                      <img style="clip-path: circle(50%);" class="w-6 h-6" src={`https://financialmodelingprep.com/image-stock/${item?.symbol}.png`} loading="lazy" />
                    </div>
          ...
          <div class="text-start text-sm font-semibold text-slate-400 mb-2">
            ...
            {#each searchResults as item}
              <li>
                <!-- svelte-ignore a11y-click-events-have-key-events -->
                <!-- svelte-ignore a11y-label-has-associated-control -->
                <label data-sveltekit-preload-data="false" on:click={() => (searchBarTicker(item?.symbol, item?.type))} class="mb-2 {item?.symbol === focusedSuggestion ? 'shake-ticker cursor-pointer flex justify-start items-center p-2 text-white bg-[#2a2e39] bg-opacity-[0.25] rounded group' : 'cursor-pointer mb-2 bg-[#0d1117] bg-opacity-[0.4] rounded-lg flex justify-start items-center p-2 text-white group'}">
                  <div class="flex flex-row items-center w-full">
