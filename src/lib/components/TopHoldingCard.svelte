...
$: {
    if($etfTicker && typeof window !== 'undefined')
    {
      ...
      <div class="lg:rounded-2xl shadow-lg bg-[#0d1117] lg:bg-[#0d1117] lg:border lg:border-slate-800 h-auto {$screenWidth <= 800 ? 'w-screen pt-16' : ''} lg:w-96">
        ...
        <div class="w-auto lg:w-full p-1 flex-1 flex flex-wrap pb-5">
            ...
            <table class="table table-sm table-compact mt-3 text-start flex justify-start items-center w-full px-3 m-auto">
              <thead>
                <tr class="border-b border-blue-400">
                  <th class="text-white font-semibold text-sm text-start bg-[#0d1117] lg:bg-[#0d1117] border-b border-blue-400">Company</th>
                  <th class="text-white font-semibold text-sm text-start bg-[#0d1117] lg:bg-[#0d1117] border-b border-blue-400">Market Value</th>
                  <th class="text-white font-semibold text-sm text-end bg-[#0d1117] lg:bg-[#0d1117] border-b border-blue-400">Portfolio</th>
                </tr>
              </thead>
              <tbody>
                {#each topHoldingList?.slice(0,5) as item}
                {#if item?.asset !== null}
                <tr on:click={() => stockSelector(item?.asset)} class="lg:shake-ticker text-white cursor-pointer lg:hover:bg-[#245073] lg:hover:bg-opacity-[0.2] bg-[#0d1117] lg:bg-[#0d1117] border-b border-[#000] lg:border-[#202020]">
          ...
          <label for="topHoldingModal" on:click={() => goto(`/etf/${$etfTicker}/holdings`)} class="rounded-lg cursor-pointer w-11/12 md:w-3/4 lg:w-11/12 py-2 h-full mt-8 lg:mt-6 text-[1rem] text-center font-semibold text-white m-auto hover:bg-[#161b22] bg-[#161b22] transition duration-100 ease-in">
            View All Holdings
          </label>
