...
$: {
    if($etfTicker && typeof window !== 'undefined')
    {
      ...
      <div class="lg:rounded-2xl shadow-lg bg-[#111111] lg:bg-[#111111] lg:border lg:border-slate-800 h-auto {$screenWidth <= 800 ? 'w-screen pt-16' : ''} lg:w-96">
            ...
            <table class="table table-sm table-compact mt-3 text-start flex justify-start items-center w-full px-3 m-auto">
              <thead>
                <tr class="border-b border-blue-400">
                  <th class="text-white font-semibold text-sm text-start bg-[#111111] lg:bg-[#111111] border-b border-blue-400">Company</th>
                  <th class="text-white font-semibold text-sm text-start bg-[#111111] lg:bg-[#111111] border-b border-blue-400">Market Value</th>
                  <th class="text-white font-semibold text-sm text-end bg-[#111111] lg:bg-[#111111] border-b border-blue-400">Portfolio</th>
                </tr>
              </thead>
              <tbody>
                {#each topHoldingList?.slice(0,5) as item}
                {#if item?.asset !== null}
                <tr on:click={() => stockSelector(item?.asset)} class="lg:shake-ticker text-white cursor-pointer lg:hover:bg-[#245073] lg:hover:bg-opacity-[0.2] bg-[#111111] lg:bg-[#111111] border-b border-[#000] lg:border-[#202020]">
