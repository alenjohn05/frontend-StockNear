...
<div class="p-2 w-full max-w-xl mr-auto overflow-y-scroll {commentId?.length !== 0 ? '-ml-2' : ''}">
  <textarea
    ...
    class="rounded-lg text-sm  {expandField ? 'min-h-24 h-auto border-[#1C4090]' : 'h-12  border-gray-500'} overflow-hidden sm:hover:border-[#1C4090] sm:hover:ring-1 transition sm:ease-out placeholder-gray-500 w-full bg-[#0d1117] text-white border border-1 ring-2 sm:ring-0 ring-[#1C4090]"
    ...
    <div class="flex flex-col">
      ...
      -->
        ...
        {#if isLoaded}
          <label on:click={handleComment} class="inline-flex justify-end items-center bg-[#161b22] {inputValue.length !== 0 ? 'opacity-100 cursor-pointer' : 'opacity-60'}  py-2.5 px-4 text-xs font-medium text-center text-white rounded-lg focus:ring-purple-300">
              Post
          </label>
        {:else}
        <label class="inline-flex justify-end items-center bg-[#161b22] opacity-60 py-2.5 px-4 text-xs font-medium text-center text-white rounded-lg focus:ring-purple-300">
          Post
        </label>
        {/if}
...
<section class="border-1 border-t border-slate-800 {imageInput?.length !== 0 ? 'min-h-44' : expandField ? 'min-h-40' : 'min-h-24'} rounded-none fixed z-20 w-full -translate-x-1/2 bg-[#0d1117] bottom-0 left-1/2">
  ...
  <textarea
    ...
    <div class="flex flex-col">
      <div class="flex flex-row items-center mt-4">
        ...
        {#if isLoaded}
        <label on:click={handleComment} class="mr-5 bg-[#161b22] {inputValue.length !== 0 ? 'opacity-100' : 'opacity-60'}  py-2.5 px-4 text-xs font-medium text-center text-white rounded-lg focus:ring-purple-300">
            Post
        </label>
        {:else}
        <label class="mr-5 bg-[#161b22] opacity-60 py-2.5 px-4 text-xs font-medium text-center text-white rounded-lg focus:ring-purple-300">
          Post
        </label>
        {/if}
