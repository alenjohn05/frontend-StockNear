    ...
    <main class="overflow-hidden ">
        ...
        <div class="flex flex-row items-center">
            ...
            {#if data?.user?.tier === 'Pro'}
                ...
                <div class="p-3 sm:p-0 mt-2 pb-8 sm:pb-2 rounded-lg bg-[#0d1117] sm:bg-[#0d1117]">
                          ...
                          <div class="relative size-14 ml-auto">
                            <svg class="size-full w-14 h-14" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg">
                              ...
                              <g class="origin-center -rotate-90 transform">
                                <circle cx="18" cy="18" r="16" fill="none" class="stroke-current text-[#FFBE00]" stroke-width="3" stroke-dasharray="100" stroke-dashoffset={putCallRatio >=1 ? 0 : 100-(putCallRatio*100)?.toFixed(2)}></circle>
                              </g>
                            </svg>
                            <!-- Percentage Text -->
                    ...
                    <table class="table table-sm table-compact w-full">
                      <thead>
                        <tr class="border-b border-blue-400">
                          <th class="text-white shadow-md font-semibold text-sm text-start bg-[#0d1117]">Institute</th>
                          <th class="text-white shadow-md font-semibold text-sm text-start bg-[#0d1117]">Ownership</th>
                          <th class="text-white shadow-md font-semibold text-sm text-end hidden sm:table-cell bg-[#0d1117]">Shares</th>
                          <th class="text-white shadow-md font-semibold text-sm text-end hidden sm:table-cell bg-[#0d1117]">Market Value</th>
                          <th class="text-white shadow-md font-semibold text-sm text-end  bg-[#0d1117]">Portfolio</th>
                        </tr>
                      </thead>
                      <tbody>
                        {#each (showFullStats ? shareholderList?.slice(0,10) : shareholderList?.slice(0,3)) as item,index}
                        {#if item?.investorName?.length > 0}
                        <tr on:click={() => goto('/hedge-funds/'+item?.cik)} class="border-y border-gray-800 odd:bg-[#0d1117] {index === 2 && !showFullStats && shareholderList?.length > 3 ? 'opacity-[0.5]' : '' } sm:hover:bg-[#245073] sm:hover:bg-opacity-[0.2] bg-[#0d1117] border-b-[#0d1117] cursor-pointer">
