...
<dialog id="tagSearchBarModal" class="modal modal-top ">
  ...
  <div class="modal-box overflow-hidden rounded-xl bg-[#2a2e39] sm:my-8 sm:m-auto sm:h-auto w-full sm:w-1/2 2xl:w-1/3 " >
    ...
    <div class="mb-3 last:mb-0 mt-3">
      ...
      <ul class="text-sm" >
          {#if !showSuggestions }
            {#each popularList as item}
              <li>
                <a data-sveltekit-preload-data="false" on:click={() => popularTicker(item?.symbol, item?.type) } class="mb-2 {item?.symbol === focusedSuggestion ? 'shake-ticker cursor-pointer flex justify-start items-center p-2 text-white bg-[#2a2e39] bg-opacity-[0.25] rounded group' : 'cursor-pointer bg-[#0d1117] rounded-lg flex justify-start items-center p-2 text-white group'} w-full">
                  <div class="flex flex-row items-center w-full">
                    <div class="rounded-full w-10 h-10 relative bg-[#0d1117] flex items-center justify-center">
                      <img style="clip-path: circle(50%);" class="w-6 h-6" src={`https://financialmodelingprep.com/image-stock/${item?.symbol}.png`} loading="lazy" />
                    </div>
