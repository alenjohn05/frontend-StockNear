    ...
    <div class="w-full overflow-hidden">
      <Lazy height={300} fadeOption={{delay: 100, duration: 500}} keep={true}>
        <table class="table table-sm table-compact w-full mb-5 mt-5">
          ...
          <thead>
            <tr>
              <th class="text-white text-sm font-medium bg-[#131722] font-semibold">Name</th>
              <th class="text-white text-sm font-medium bg-[#131722] font-semibold text-end">Value</th>
              <th class="text-white text-sm font-medium bg-[#131722] font-semibold text-end">Signal</th>
            </tr>
          </thead>
          <tbody>
            {#each (showFullStats ? signalList : signalList?.slice(0, 3)) as item,index}
            <tr class="border-y border-gray-800 odd:bg-[#131722] {index === 2 && !showFullStats && signalList?.length > 2 ? 'opacity-[0.3]' : '' }">
