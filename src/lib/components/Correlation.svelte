<script lang="ts">
  ...
  if ($screenWidth < 640)
  {
    ...
    <Lazy height={300} fadeOption={{delay: 100, duration: 500}} keep={true}>
      ...
      <div class="shadow-lg bg-[#0d1117] w-full rounded-lg p-4 sm:p-3 mb-5  flex flex-row items-center {index === 0 ? 'mt-4' : ''} {index === 2 && !showFullStats && correlationList?.length > 2 ? 'opacity-[0.3]' : '' }">

        <div on:click={() => stockSelector(item?.symbol)} class="flex-shrink-0 mr-3 rounded-full w-8 h-8 sm:w-10 sm:h-10 relative bg-[#0d1117]">
            <img
