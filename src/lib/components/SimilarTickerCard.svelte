...
export let similarstock;
  ...
  <div class="sm:rounded-2xl shadow-lg bg-[#0d1117] sm:bg-[#0d1117] sm:border sm:border-slate-800 h-auto {$screenWidth < 640 ? 'w-screen pt-16' : ''} md:w-96">
        ...
        <table class="table table-sm table-compact mt-3 text-start flex justify-start items-center w-full px-3 m-auto">
          <thead>
            <tr class="border-b border-blue-400">
              <th class="text-white font-semibold text-sm text-start bg-[#0d1117] sm:bg-[#0d1117] border-b border-blue-400">Company</th>
              <th class="text-white font-semibold text-sm text-center bg-[#0d1117] sm:bg-[#0d1117] border-b border-blue-400">Market Cap</th>
              <th class="text-white font-semibold text-sm text-end bg-[#0d1117] sm:bg-[#0d1117] border-b border-blue-400">Avg Volume</th>
            </tr>
          </thead>
          <tbody>
            {#each similarstock as item, index}
            <tr on:click={() => stockSelector(item?.symbol)} class="shake-ticker text-white cursor-pointer sm:hover:bg-[#245073] sm:hover:bg-opacity-[0.2] bg-[#0d1117] sm:bg-[#0d1117] border-b border-[#000] sm:border-[#202020]">
              {#if index <=6}
                ...
                <td class="text-gray-200">
                  <div class="flex flex-row items-center">
                    ...
                    <div class="flex flex-col ml-3 w-full">
                      <span class="text-[#FFBE00] text-sm font-medium">{item?.symbol}</span>
  ...
  <!--End Header-->
    ...
    <div class="flex justify-start items-center w-full m-auto mt-10 overflow-hidden">
      <table class="table table-sm table-compact mt-3 text-start flex justify-start items-center w-full px-3 m-auto">
        <thead>
          <tr class="border-b border-blue-400">
            <th class="text-white font-bold text-sm text-start bg-[#0d1117] border-b border-blue-400">Company</th>
            <th class="text-white font-bold text-sm text-center bg-[#0d1117] border-b border-blue-400">Market Cap</th>
            <th class="text-white font-bold text-sm text-end bg-[#0d1117] border-b border-blue-400">Avg Volume</th>
          </tr>
        </thead>
        <tbody>
          {#each similarstock as item, index}
          <tr on:click={() => stockSelector(item?.symbol)} class="text-white cursor-pointer bg-[#0d1117] border-b border-[#000]">
            {#if index <=6}
