            ...
            <table class="table table-sm table-compact text-start flex justify-start items-center w-full px-3 m-auto">
              <thead>
                <tr class="border-b border-blue-400">
                  <th class="text-white font-semibold text-sm text-start bg-[#131722] sm:bg-[#131722] border-b border-blue-400">Company Name</th>
                  <th class="text-white font-semibold text-sm text-center bg-[#131722] sm:bg-[#131722] border-b border-blue-400">Total Assets</th>
                  <th class="text-white font-semibold text-sm text-end bg-[#131722] sm:bg-[#131722] border-b border-blue-400">% of Fund</th>
                </tr>
              </thead>
              <tbody>
                {#each topETFHolder as item, index}
                <tr on:click={() => etfSelector(item?.symbol)} class="text-white cursor-pointer sm:hover:bg-[#245073] sm:hover:bg-opacity-[0.2] bg-[#131722] sm:bg-[#131722] border-b border-[#000] sm:border-[#202020]">
                  {#if index <=6}
          ...
          <table class="table table-sm table-compact mt-3 text-start flex justify-start items-center w-full px-3 m-auto">
            <thead>
              <tr class="border-b border-blue-400">
                ...
                <th class="text-white font-bold text-sm text-center bg-[#131722] border-b border-blue-400">Total Assets</th>
                <th class="text-white font-bold text-sm text-end bg-[#131722] border-b border-blue-400">% of Fund</th>
              </tr>
            </thead>
            <tbody>
              {#each topETFHolder as item, index}
              <tr on:click={() => etfSelector(item?.symbol)} class="text-white cursor-pointer bg-[#131722] border-b border-[#000]">
                {#if index <=6}
