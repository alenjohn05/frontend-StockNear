...
<section class="w-full max-w-4xl overflow-hidden m-auto min-h-screen pt-10 mb-40">
    ...
    {#if isLoaded}
      ...
      <div class="flex justify-start items-center ml-3 mb-10">
        <label for="sortByModal" class="cursor-pointer bg-[#131722] flex flex-row items-center">
      ...
      <div class="w-screen sm:w-full rounded-lg">
        <table class="table table-sm table-compact rounded-none sm:rounded-md w-full bg-[#131722] border-bg-[#131722] m-auto mt-4 ">
          ...
          <tbody class="p-0">
            ...
            <!-- row -->
            <tr on:click={() => goto(`/${item?.type === 'stock' ? 'stocks' : item?.type === 'etf' ? 'etf' : 'crypto'}/${item?.symbol}`)} class="sm:hover:bg-[#245073] sm:hover:bg-opacity-[0.2] odd:bg-[#131722] border-b-[#131722] cursor-pointer">
              

              <td class="hidden sm:table-cell text-[#FFBE00] text-sm text-start border-b-[#131722]">
                {item?.symbol}
              </td>
              ...
              <td class="text-white text-sm text-center border-b-[#131722] {$screenWidth < 640 && sortBy !== 'EPS' ? 'hidden' : ''}">
                {item?.eps !== null ? item?.eps?.toFixed(2) : '-'}
            </td>

            <td class="text-white text-sm text-center border-b-[#131722] {$screenWidth < 640 && sortBy !== 'PE Ratio' ? 'hidden' : ''}">
                {item?.pe !== null ? item?.pe?.toFixed(2) : '-'}
            </td>

            <td class="text-white text-sm text-center border-b-[#131722] {$screenWidth < 640 && sortBy !== 'Volume' ? 'hidden' : ''}">
            ...
            <td class="text-white text-sm text-center border-b-[#131722] {$screenWidth < 640 && (sortBy !== 'Market Cap' && sortBy !== 'Change' && sortBy !== 'Price' && sortBy !== 'Name: A-Z') ? 'hidden' : ''}">
